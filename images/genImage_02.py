...
def makeImage(height, width, imageName):
    ...
    for d in range(N):
        ...
        x1, y1 = helper.lemniscate(ang1, radius)
        x2, y2 = helper.lemniscate(ang2, radius)
       
        rotateAngle = -pi/4
        coordinates.append(
            (
                helper.rotate(x1, y1, rotateAngle), 
                helper.rotate(x2, y2, rotateAngle), 
            )
        )
    ...
    for coords in coordinates:
        ...
        ctx.move_to(p0[0] * scale + cx, 
                    p0[1] * scale + cy)
        ctx.line_to(p1[0] * scale + cx, 
                    p1[1] * scale + cy)
        ctx.stroke()
