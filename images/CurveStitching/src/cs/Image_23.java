...
public class Image_23 extends CSDP {
	...
	static final Color FOREGROUND = new Color(0, 128, 128);
	...
	public Point2D.Double getRandomPoint(Point2D.Double a, Point2D.Double b, double cx, double cy) {
            double dist = Math.sqrt(Math.pow(a.x - b.x, 2) + 
                                    Math.pow(a.y - b.y, 2));
            double circleRadius = dist/2;
            double circleCx = (a.x+b.x)/2;
            double circleCy = (a.y+b.y)/2;

            double randomTheta = rand.nextDouble() * 2 * Math.PI;
            return circle(randomTheta, circleRadius,
                          circleCx + cx, circleCy + cy);
		}
	}
	...
	public static void main(String[] args) throws IOException {
		...
		Image_23 imageCDSP = new Image_23();
